...
// Assign the methods in the prototypeDino to all objects created
// with DinoConstructor
DinoConstructor.prototype = prototypeDino;
...
function createDinoArray(units) {
    const dinos = rawDinosaursData();
    const dino_array = [];

    dinos.forEach(function (dino) {
        dino_array.push(new DinoConstructor(dino, units));
    });
    ...
    dino_array.splice(4, 0, 'human placeholder');
 ...
 */
function createDinoElement(dino_data, human_data) {
    let fact;
    ...
    const randomNumber = dino_data.species === 'Pigeon' ? 2 : Math.round(Math.random() * 5);

    switch (randomNumber) {
        case 0:
            fact = `The ${dino_data.species} lived in ${dino_data.where}.`;
            break;
        case 1:
 ...
 * @returns {Element} An element to be added to the grid in the UI
 */
function createHumanElement(human_data) {
 ...
 */
function updateUI(dino_array, human_data) {
    ...
    for (let i = 0; i < 9; i++) {
        ...
        let gridSquare = i === 4 ? createHumanElement(human_data) : createDinoElement(dino_array[i], human_data);
...
function clicked(e) {
    ...
    if (human_data.name === "") {
        ...
        return;
    } else if (human_data.height < 1) {
        errorMessage.innerHTML = '<p>Please enter a height more than 0</p>';
        return;
    } else if (human_data.weight < 1) {
        errorMessage.innerHTML = '<p>Please enter a weight more than 0</p>';
        return;
    }

    const dino_array = createDinoArray(human_data.units);

    updateUI(dino_array, human_data);
}
